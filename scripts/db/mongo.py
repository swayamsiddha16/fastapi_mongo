...
def view_student(id:int):
    student = mongoclient.find_one({"id":id},DBConf.DB_DATABASE,DBConf.DB_COLLECTION)
    if student:
        del student["_id"]
        return student
    else: 
...
def view_studentbykey(key:str,val):
    students = mongoclient.find_many({key:val},DBConf.DB_DATABASE,DBConf.DB_COLLECTION)
    students_list = []
